        ...
        <div class="flex-grow-1">
            ...
            <form id="formMainMenu" action="" method="post">
                <div class="row row-cards">
                    <div class="col-sm-12">
                        <div class="mb-3">
                            <label class="form-label">Nama User Level</label>
                            <input type="text" class="form-control " name="user_level_name" required>
    ...
    $(document).ready(function() {
        ...
        function loadForm() {
            ...
            let biodata_ref = `
                <div class="col-sm-12">
                    <div class="mb-3">
                        <label class="form-label">Biodata Ref</label>
                        <select class="form-select" name="biodata_ref_id" id="biodata_ref_id" required>
                            <option value="">Pilih Biodata Ref</option>
                        </select>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
            `;
            $('#additionalSelectContainer').append(biodata_ref);
            let app = `
                <div class="col-sm-12">
                    <div class="mb-3">
            ...
            `;
            $('#additionalSelectContainer').append(app);
        }
            ...
            return new Promise((resolve, reject) => {
                $.ajax({
                    ...
                    success: function(response) {
                        let biodatRefOptions = '<option value="">Pilih Biodata Ref</option>';
                        response.data.biodata_ref.forEach(function(biodata) {
                            biodatRefOptions +=
                                `<option value="${biodata.id}">${biodata.biodata_ref_name}</option>`;
                        });
                        $('[name="biodata_ref_id"]').html(biodatRefOptions);

                        let appOptions = '<option value="">Pilih Aplikasi</option>';
                        response.data.app.forEach(function(app) {
                            appOptions +=
                                `<option value="${app.id}">${app.app_name}</option>`;
                        });
                        $('[name="app_id"]').html(appOptions);
            ...
            try {
                ...
                $.ajax({
                    ...
                    success: function(response, textStatus, xhr) {
                        ...
                        $('#formMainMenu [name="user_level_name"]').val(data.user_level_name);
                        $('#formMainMenu [name="biodata_ref_id"]').val(data.biodata_ref_id).trigger('change');
                        $('#formMainMenu [name="app_id"]').val(data.app_id).trigger('change');
